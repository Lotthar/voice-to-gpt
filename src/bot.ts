...
import { botChatGptModelChanged, botSystemMessageChanged } from "./chatgpt-util.js";
import {
...
discordClient.on(Events.MessageCreate, async (message: Message) => {
  try {
    ...
    if (botIsMentioned(message)) {
      ...
      const sendMessagePromise = sendMessageToProperChannel(answer, message.channelId).then(() => {
        messageSent = true;
      });
      await Promise.all([typingPromise, sendMessagePromise]);
    }
  } catch (error) {
