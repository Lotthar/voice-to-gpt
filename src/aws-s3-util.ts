...
export const uploadFileToS3 = async (key: string, content: string): Promise<PutObjectCommandOutput> => {
  ...
  try {
    ...
    console.error(`Error uploading data to AWS S3 bucket for channel: ${currentChannelId}:`, error);
    throw error;
  }
};
...
export const downloadFileFromS3 = async (key: string): Promise<Readable> => {
  ...
  try {
    ...
    console.error(`Error downloading data from AWS S3 bucket for channel: ${currentChannelId}:`, error);
    throw error;
  }
};
